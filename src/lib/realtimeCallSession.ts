...
You also understand Korean. If they speak Korean, follow them. You may answer in Korean when they clearly want that, then come back to English unless they stay in Korean.
...
Speak ${language} by default in short, natural spoken turns.
If they switch into another language you understand, follow them; prefer ${language} unless they stay in the other language.
...
export function realtimeCallSessionConfig(targetLanguage: LearningLanguageCode) {
  return {
    type: "realtime" as const,
    model: REALTIME_CALL_MODEL,
    instructions: realtimeCallInstructions(targetLanguage),
    audio: {
      output: {
