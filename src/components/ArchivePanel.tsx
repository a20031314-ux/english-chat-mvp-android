...
export function ArchivePanel({
  ...
  return (
    ...
    >
      ...
      />
      <aside
        className={`absolute left-0 top-0 h-full w-[88%] max-w-sm overflow-y-auto border-r border-white/10 bg-[#121212] p-4 shadow-2xl transition-transform duration-300 ease-out ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <header className="mb-4 flex items-center justify-between">
          <h2 className="text-base font-semibold text-slate-100">{ui.archive}</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <section>
          <div className="mb-2 flex items-center justify-between">
            <h3 className="text-sm font-semibold text-slate-100">{ui.sessionsTab}</h3>
          ...
          {conversationSessions.length === 0 ? (
            <p className="rounded-lg border border-dashed border-white/15 p-3 text-xs text-slate-300">
              {ui.sessionsEmpty}
            </p>
          ) : (
            <div className="space-y-2">
              {conversationSessions.map((session) => (
                <article
                  key={session.id}
                  className="rounded-lg border border-white/10 bg-[#121212] p-3 text-xs"
                >
